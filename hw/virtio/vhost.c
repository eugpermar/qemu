...
static int vhost_sw_live_migration_thread(struct vhost_dev *dev,
                                          bool enable_lm)
{
    if (enable_lm) {
        return vhost_sw_live_migration_thread_start(dev);
    } else {
        return vhost_sw_live_migration_thread_stop(dev);
    }
}
...
static void vhost_sw_lm_global_start(MemoryListener *listener)
{
    ...
    r = vhost_migration_log(listener, true, vhost_sw_live_migration_thread);
    if (r < 0) {
        abort();
    }
}
...
static void vhost_sw_lm_global_stop(MemoryListener *listener)
{
    ...
    r = vhost_migration_log(listener, false, vhost_sw_live_migration_thread);
    if (r < 0) {
        abort();
    }
}
...
{
    assert(hdev->vhost_ops->vhost_reset_device);
    assert(hdev->vhost_ops->vhost_get_vring_base);
